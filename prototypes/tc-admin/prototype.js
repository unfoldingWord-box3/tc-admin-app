...
let account = null, connected = false, loading = true, loadError = '', loadEpoch = 0, configured = false, env = { name: 'QA', host: 'qa.door43.org', development: true };
...
async function bootstrap() {
  try {
    const session = await api('/api/session'); connected = session.connected; configured = session.configured; if (session.environment) env = session.environment;
...
function signInView() {
  return `<section class="signin panel"><img src="design-system/assets/translationcore-logo.png" alt="" width="56" height="56"><div class="overline">Your translation portfolio</div><h1>Your projects.<br>A clearer perspective.</h1><p>Sign in with Door43 ${env.name} to see the projects you can manage, organized around your team.</p>${loadError?`<div class="callout error" role="alert">${esc(loadError)}</div>`:''}<button class="primary" onclick="connectDoor43()" ${loading||!configured?'disabled':''}>${loading?'Connecting…':`Sign in with Door43 ${env.name} →`}</button><p class="muted">${configured?`Connected to ${env.host} · Only read access is requested.`:`${env.name} sign-in is being configured.`}<br>${env.development?'Development environment · QA data may be reset.':'Production Door43 · read-only prototype.'}</p></section>`;
}
function liveStatus() {
  return `<div class="heading"><div><div class="overline">Your translation portfolio</div><h1>Projects</h1><p>${loading?`Loading your writable Door43 ${env.name} projects…`:'Your portfolio could not be loaded.'}</p></div></div>${loadError?`<div class="callout error" role="alert">${esc(loadError)}</div>${btn('Retry','loadLiveProjects()','primary')}`:'<div class="panel"><p>Your account permissions determine which projects appear here.</p></div>'}`;
}
