...
- Create a Bible or Open Bible Stories repository with valid Scripture Burrito metadata.
...
- Maintain project metadata without hand-writing it.
...
Translation managers should not need to inspect repositories one at a time, hand-write metadata, or depend on unfoldingWord staff to perform routine releases. Those activities consume support capacity and make release timing dependent on specialized knowledge.
...
- A repository's metadata and health state become understandable to its manager.
...
4. The manager creates or opens a project, uploads files, and maintains its metadata.
...
- Every action is a reviewable plan before it is a write, and every write leaves a receipt, so a manager, a support engineer, or an agent acting for a manager can see what will happen and what happened (ADR 0011).
